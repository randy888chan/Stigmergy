...
describe("ExecutiveSummary", () => {
  test("shows a loading state initially", () => {
    global.fetch = mock.fn(() => new Promise(() => {})); // A promise that never resolves
  ...
  test("displays an error message if the API call fails", async () => {
    global.fetch = mock.fn(() => Promise.resolve({ ok: false, statusText: "Server Error" }));
    render(<ExecutiveSummary />);
    await waitFor(() => {
      expect(screen.getByText(/Error: Failed to fetch executive summary/)).toBeInTheDocument();
    });
  });
});
