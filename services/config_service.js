...
class ConfigService {
  ...
  async _loadEnv() {
    ...
    if (nodeEnv !== 'test' && process.env.USE_MOCK_AI !== 'true') {
  ...
  _validateConfig(config) {
    ...
    if (!hasAnyProvider) {
      errors.push(
        "At least one AI provider must be configured (e.g., GOOGLE_API_KEY or OPENROUTER_API_KEY from .env or Doppler)."
      );
    }
  ...
  async _loadAndValidate() {
    ...
    const dynamicConfig = {
      ...
      model_tiers: {
        reasoning_tier: {
          ...staticConfig.model_tiers.reasoning_tier,
        ...
        strategic_tier: {
          ...staticConfig.model_tiers.strategic_tier,
        ...
        execution_tier: {
          ...staticConfig.model_tiers.execution_tier,
        ...
        utility_tier: {
          ...staticConfig.model_tiers.utility_tier,
      ...
      provider_config: {
        ...
        cost_optimization: {
          ...
          max_cost_per_request: parseFloat(process.env.MAX_COST_PER_REQUEST) || 0.1,
        },
        quality_thresholds: {
  ...
  getConfig() {
    if (!this.isInitialized) {
      ...
      throw new Error(
        "ConfigService has not been initialized. Please call await configService.initialize() at application startup."
      );
    }
    return this.config;
  }
