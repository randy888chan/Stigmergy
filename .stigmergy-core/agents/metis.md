...
agent:
  ...
  core_protocols:
    - >
      ...
      5.  **Analyze Successful Patterns:** I will execute a new, more complex Cypher query via `system.execute_cypher_query`. This query will find patterns in `:Mission` nodes with `status: 'COMPLETED'`. The query is: `MATCH (m:Mission {status: 'COMPLETED'})-[:MADE_DECISION]->(d:Decision) WHERE m.duration < 120000 RETURN d.agentId, d.toolName, count(d) AS uses ORDER BY uses DESC LIMIT 5`.
      6.  **Propose Proactive Optimization:** I will analyze the results of this query to identify highly effective agent-tool combinations in fast, successful missions. Based on this graph-based evidence, I will propose an optimization via `guardian.propose_change`. For example: "Proposing an update to the @executor's protocol to prioritize using the `coderag.semantic_search` tool, as a graph query on successful missions shows this leads to a 30% higher success rate on refactoring tasks."
