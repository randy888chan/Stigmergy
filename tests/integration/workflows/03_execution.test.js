...
`;
  ...
  test('Dispatcher should read plan and delegate to executor', async () => {
    const dispatcherReadsPlan = { text: "Reading plan...", toolCalls: [{ toolName: 'file_system.readFile', args: { path: 'plan.md' }, toolCallId: '1' }], finishReason: 'tool-calls' };
    const dispatcherReadsFile = { text: "Reading file...", toolCalls: [{ toolName: 'file_system.readFile', args: { path: 'src/example.js' }, toolCallId: '2' }], finishReason: 'tool-calls' };
    const dispatcherDelegates = { text: "Delegating...", toolCalls: [{toolName: 'stigmergy.task', args: { subagent_type: '@executor', description: 'Create file src/example.js with content: console.log("hello");' }, toolCallId: '3'}], finishReason: 'tool-calls' };
    const executorWritesFile = { text: "Writing file...", toolCalls: [{ toolName: 'file_system.writeFile', args: { path: 'src/example.js', content: 'console.log("hello");' }, toolCallId: '4' }], finishReason: 'tool-calls' };
    const executorFinishes = { text: "File written.", toolCalls: [], finishReason: 'stop' };
    const dispatcherUpdatesStatus = { text: "Updating status.", toolCalls: [{toolName: 'system.updateStatus', args: { newStatus: 'PLAN_EXECUTED' }, toolCallId: '5'}], finishReason: 'tool-calls' };
...
`;
    ...
    const finalContent = await mockFs.promises.readFile(path.join(projectRoot, '.stigmergy/sandboxes/executor/src/example.js'), 'utf-8');
