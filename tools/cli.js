...
program
  .name('pheromind-cli')
  .description('Pheromind: CLI for managing the Autonomous AI Development Swarm.')
  .version(require('../package.json').version);

program
  .command('build')
  .description('Build web bundles for agents and teams from the bmad-core source.')
...
program
  .command('build:web')
  .description('Build only the team bundles required for the Web UI (e.g., Gemini, Claude).')
  .action(async () => {
      const builder = new WebBuilder({ rootDir: process.cwd() });
      try {
          console.log('Cleaning output directory...');
...
program
  .command('list:agents')
  .description('List all available agents in the bmad-core.')
  .action(async () => {
    const builder = new WebBuilder({ rootDir: process.cwd() });
