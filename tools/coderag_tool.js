 ...
 */
export const createCoderagTool = (unifiedIntelligenceService) => ({
  /**
   ...
   */
  scan_codebase: async ({ project_root }) => {
    try {
   ...
   */
  semantic_search: async ({ query, project_root }) => {
    if (!query) {
   ...
   */
  find_architectural_issues: async () => {
    try {
