...
async function configureIde(coreDestDir) {
  ...
  const manifestPath = path.join(coreDestDir, "system_docs", "02_Agent_Manifest.md");
  const manifestContent = await fs.readFile(manifestPath, "utf8");
  const manifest = yaml.load(manifestContent.split('---')[0]);
  ...
  for (const agentEntry of manifest.agents) {
    ...
    const agentMdPath = path.join(coreDestDir, 'agents', `${agentEntry.id}.md`);
    ...
    modes.push({
      slug: agentEntry.alias, name: `${agentIcon} ${agentName}`, roleDefinition,
      api: { url: `${ENGINE_URL}/api/chat`, method: "POST", include: ["history"], static_payload: { agentId: agentEntry.id } },
      groups: ['read'],
    });
  }

  modes.push(
    { slug: "system-start", name: "🚀 Start Project", roleDefinition: "Provide a high-level goal to start a new project.", api: { url: `${ENGINE_URL}/api/system/start`, method: "POST", body: '{"goal": "{{prompt}}"}' }, groups: ["command"] },
    { slug: "system-pause", name: "⏸️ Pause Engine", roleDefinition: "Pause the autonomous engine.", api: { url: `${ENGINE_URL}/api/control/pause`, method: "POST" }, groups: ["command"] },
    { slug: "system-resume", name: "▶️ Resume Engine", roleDefinition: "Resume the autonomous engine.", api: { url: `${ENGINE_URL}/api/control/resume`, method: "POST" }, groups: ["command"] }
  );
...
async function run() {
  ...
  try {
    ...
    // THIS IS THE FIX: Throw an error instead of exiting the process.
    throw error;
  }
}
