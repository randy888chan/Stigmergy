...
// Load .env from the project directory where the command is run
 ...
 * @returns {object|null} The parsed YAML config or null.
 */
...
/**
 * Extracts the full persona and core protocols to be used as the IDE role definition.
 * @param {object} agentConfig - The parsed agent YAML config.
 * @returns {string} The complete role definition for the agent.
 */
function buildRoleDefinition(agentConfig) {
  ...
  if (core_protocols && Array.isArray(core_protocols)) {
    definition += "\n\n--- CORE PROTOCOLS ---\n";
    definition += core_protocols.map(p => `- ${p}`).join("\n");
  }
  return definition;
}
