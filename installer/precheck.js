...
export function checkWindowsPermissions() {
...
const CHECKS = [
  ...
  { name: "Disk Space", check: async () => (await checkDiskSpace("/")).free > 500e6 },
  {
    ...
    check: async (path = process.cwd()) => fs.promises.access(path, fs.constants.W_OK),
  },
  { name: "Ports Available", ports: [3000, 7687] },
  {
    name: "Windows Permissions",
    check: checkWindowsPermissions,
  },
];
